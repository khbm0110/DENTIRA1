      ...
      <div className="max-w-7xl mx-auto px-6">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-8">
          {displayDoctors.map((doctor, index) => (
            <AnimateIn key={index} delay={index * 0.08} className="bg-surface rounded-3xl overflow-hidden group shadow-[0_4px_20px_rgb(0,0,0,0.03)] hover:shadow-[0_12px_40px_rgb(0,0,0,0.08)] transition-all duration-300">
              ...
              <div className="p-3 sm:p-6 text-center">
                <h3 className="font-headline font-bold text-sm sm:text-xl mb-0.5 sm:mb-1 text-on-surface truncate">{doctor.name}</h3>
                <p className="text-primary text-xs sm:text-sm font-semibold mb-2 sm:mb-4 truncate">
                  {doctor.title}
                </p>
        ...
        <div className="mt-8 md:mt-16 text-center">
          <button className="bg-primary text-white px-8 py-4 rounded-full font-bold text-lg inline-flex items-center justify-center gap-2 shadow-[0_8px_25px_-5px_rgb(0,105,113,0.5)] hover:shadow-[0_12px_35px_-5px_rgb(0,105,113,0.6)] hover:-translate-y-1 transition-all duration-300">
            {currentLang === 'ar' ? 'شاهد كل الأطباء' : 'Voir tous les médecins'}
          </button>
        </div>
      </div>
    </section>
